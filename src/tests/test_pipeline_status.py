...
from src.utils.pipeline import record_selected, record_aborted, _post_slug, record_published
...
def test_appends_to_existing_file(tmp_path):
    pipeline = tmp_path
    events = _events_dir(tmp_path)
    (events / "260326-status.txt").write_text("1:abort\n")
    record_published("260326", 7, pipeline_dir=pipeline)
    assert (events / "260326-status.txt").read_text() == "1:abort\n7:published\n"
...
def test_raises_if_already_abort(tmp_path):
    pipeline = tmp_path
    events = _events_dir(tmp_path)
    (events / "260326-status.txt").write_text("1:abort\n")
    with pytest.raises(RuntimeError, match="abort"):
...
def test_reader_ignores_blank_and_comment_lines(tmp_path):
    ...
    record_published("260326", 5, pipeline_dir=tmp_path)
