...
# 只给"这份文件是不是漂移的新格式"这类粗判用（research_linter.lint_research 的分派
# 兜底）——E_REF_RE 本身不许放宽：它是解析用的严格判据，`_lint_extracts` 的孤儿检测
# 在用，Task 6 的 `_lint_facts` 还要用它数事实句的出处；放宽＝把全角方括号的引用当
# 合法解析，格式漂移被悄悄接纳。中文输入法全角/半角切换下 [E1] 可能整个被打成
# ［E1］，与 LOOSE_HEAD_RE 认全角左括号同一个理由——严格解析 ＋ 宽判据只用于识别
# 异常，是同一套分工。
E_REF_LOOSE_RE = re.compile(r"[\[［]E\d+[\]］]")
