...
- **带色引文必须逐字回查（用户裁定，2026-07-28，一天内三次复现）：** `source/_drafts/template.md` 规定"灰色 = 当事人/法院/官方的逐字引用"，其反向义务同样成立：**凡要上灰色的文字，必须能在研究文件（或研究文件所引原文）里逐字命中**，交稿前把每段灰字回研究文件搜一遍，搜不到就不许上灰色。红字若是对官方通报、法院认定、平台处置结论的转述，二选一：改成灰字逐字引用，或去掉颜色写成明确的转述——**不得以"近乎逐字、又有几处改写"的形态呈现**，那会让读者把你的改写当成官方原话。（例：260605-4 灰字把法官的第三人称转述包装成直接引语，原始报道均无引号；链条见 casebook）
