...
- **带色引文必须逐字回查，基准是 `## 信息来源`（用户裁定 2026-07-28；基准 2026-07-31 更正）：** 凡上灰色的文字必须逐字命中研究文件 `## 信息来源` 的引文摘录（或该来源原文），交稿前逐段回查。**基准不认 `## 事实`／`## 当事方`**——那是研究阶段的叙述性转写，照它抄"对得上"的可能是转写版。`## 信息来源` 没有逐字摘录就按缺口上报，不许自行上灰色。红字若是对官方结论的转述，二选一：改灰字逐字引用，或去色写成明确转述——**不得以"近乎逐字、又有改写"的形态呈现**。（linter --research 会 WARN 未命中的灰字；化名替换/外文译文属预期不命中，逐条确认即可）（例：260605-4；见 casebook）
