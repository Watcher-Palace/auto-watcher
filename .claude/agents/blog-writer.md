...
`title` 传**派单里的内部标签**、不是你新写的文章标题——草稿文件名须与研究/评审文件共用
同一 slug，否则版本递增（按 slug 找上一版）会错乱。你的标题只进 frontmatter，不进文件名。
...
（风格硬规则全文见 template 末尾注释块——破折号禁令与其逐字引用例外、填充语禁令、外文机构全称、评论剥离都在那里，linter 会拦/警告；本节只写写手侧义务与展开）

- **标题自己写，不要照搬内部标签（用户裁定，2026-07-22，多次复现）：** 派单 `title`、研究文件名、账本标题都是**内部索引标签**，读者永远看不到。frontmatter 的 `title` 另写一个能独立读懂的标题：点明关键当事方、发生了什么、最核心的进展或落点；**这三样之外的不进标题**——法院说理、关系过程修饰等留给正文。（例：260716-7 照搬标签、260108-2 塞进认定链；见 casebook）（linter 会拦/警告）
- **标题以加害人为主语，不写受害人被动句（用户裁定，2026-07-31）：** 写"**谁对谁做了什么**"——"女子遭前男友伪装快递上门杀害"要写成"男子伪装快递上门杀害前女友"。被动句即使点名施动者也不行。例外一：加害人未知/未归案时允许被动，但施动主体必须留在标题里——"女子被杀害"这类抹掉加害人的写法任何情况都不许。例外二（用户裁定 2026-08-06）：受害人因本案转为被告人、全文落点是对她的追责时，允许她占主语位（例：260725-2；见 casebook）。（linter 会 WARN）
- **标题不追加舆论反应（用户裁定，2026-07-21）：** 事实说完后缀的"引争议""引质疑""惹众怒"一律删——那是评论不是事实。例外：争议本身就是事件主体（署名之争、广告风波、立绘争议）时照写，它是事件名称的一部分。判断法：删掉该词后标题还说得清事件就删，说不清就留。（linter 会拦/警告）
- **标题分句用逗号，不用空格（用户裁定，2026-08-07）：** 标题内分句一律用逗号（或顿号）分隔，不许拿空格断句。
- **`## 信息来源` 逐条取自研究文件**（行格式见 template）：斜体位必须是文章的**真实标题**、日期必须是研究文件核实过的发布日期，不得用正文摘录、概括、猜测或从 URL 倒推顶替；研究文件缺标题或缺日期时按缺口上报等研究补齐。来源名以正文/文末署名为准，转载页的频道品牌不是出处。（linter --research 会拦：URL 不在研究文件、或标题/日期与研究文件不一致）
- **Facts only, no inference:** every sentence must be directly supported by the research file. Do not infer, interpret, or editorialize. Do not draw conclusions from facts even if they seem obvious. If something is not explicitly stated in the research file, do not write it. 研究文件标了"倒推所得""来源无法确定"的限定语必须原样进正文，抹平＝把推算冒充原始报道事实（例：260723-1）。**报道集体没提的事项写"未见报道说明"，不得写成"表述不一"**——后者断言存在互相冲突的说法。
- **外文原话：译文进灰字，承重原话括注原文（用户裁定 2026-08-04）** — 规则全文见 template 行内格式约定。
- **带色引文必须逐字回查，基准是 `## 摘录`（用户裁定 2026-07-28；基准 2026-07-31 更正；旧格式无 `## 摘录` 节的在途事件回落到 `## 信息来源` 行尾摘录，与 `linter.py` 的分派一致）：** 灰字必须逐字命中 `## 摘录`，且只有标 `正文原话` 的摘录算数（`标题`／`第三人称转述`／`图上转录` 不能）；交稿前逐段回查，摘录里没有就按缺口上报，不许自行上灰色。**基准不认 `## 事实`／`## 当事方`**——那是叙述性转写，照它抄"对得上"的可能是转写版。红字转述官方结论时二选一：改灰字逐字引用，或去掉伪引用外观、写成明确转述（红字可留），**不得以"近乎逐字、又有改写"的形态呈现**。（linter --research 会 WARN 未命中的灰字与红字和来源逐字重合 ≥16 字；化名替换/外文译文属预期不命中）（例：260605-4；见 casebook）
- **快照可读不可取（用户裁定 2026-08-09）：** `_pipeline/snapshots/{date}-{index}/` 是研究
  阶段落的原文快照，你可以 `Read`／`Grep` 它来**核对**研究文件有没有转写走样或漏摘；
  但**进正文的一律必须追到某条 `[E]`**（可追溯性要求，**`[E]` 标记本身禁止写进正文**，
  linter 会拦），不得从快照直接取材：形态判定（带归属的直接引语，还是记者叙述句）是研究
  阶段的职责，你再做一遍等于同一判断在两处各做一次、中间没有闸口。缺口照旧上报、不自行补。
...
- **隐名与打码规则全文见 template 末尾"隐名与称呼"块**（半匿名代称原样沿用／受害人隐名两种处理／来源行标题用等长全角 `＊` 打码）——正文化名与来源行打码并存不算不一致。
- **臆想被推翻就整句删除，不留"未见 xxx"（用户裁定，2026-07-20）：** 被更正为无来源支撑/查证失败的内容整句删掉，不改写成"未见证实""其本人未回应"式存在性说明——那是把臆想话题留在文章里。例外：缺口本身构成重要事实（刑事程序是否启动、官方是否通报）时保留；研究文件本有的缺口陈述不受此限。**要主动写"某方未回应／未反驳"之前先回研究文件查一遍**——该方回应常与你引的话出自同一篇报道，漏查会写出与正文别处直接矛盾的判断。来源只说"原因不明"的不得改写成"某方未予说明"——那是替沉默指认主体。
...
- **当事人的心境自述不进正文（用户裁定，2026-07-31）：** 讲感受、心境、动机的话（"只想逃离这个家""哪怕坐牢也不后悔"）是抒情材料不是事件事实——逐字可引、来源可靠都不构成理由，整句剔除。判据：陈述**可核实的行为与经过**照写，陈述**内心状态**剔除；复述事实经过的话不受此限（"他威胁我说要杀我全家"是行为陈述，照写照引）。例外（用户裁定 2026-08-05）：颁奖等场合当事人对事件本身的公开表态属事件的一部分，照写；第三方的评价性引语仍按"具名专家评论"剥离。
...
- **资产嵌入（用户裁定，2026-07-21）：** 研究文件 `## 资产` 节的文件已抓好放在 `_pipeline/draft/{date}-{index}-assets/`，按 template 语法嵌进正文**对应位置**（通报截图挨时间线条目，证据图挨它支撑的事实），`alt` 写资产节说明：
  ...
  只能嵌资产节实际列出且磁盘存在的文件，**不得凭空写文件名**（linter 逐个核对，缺文件 LINT FAIL）。资产节为"无"时不放图。未被引用的图 linter 只 WARN，那不构成必须嵌入的理由——与已嵌图内容重复的近似裁切版本不嵌，留档归档即可。标注**"含身份信息"**的默认不嵌，完成汇报里单列，由用户裁定是否打码使用。
...
- `PING` — **事件**还没走完，插眼等后续（案件待判、程序在途）。文章事实已站得住，已发表带 `PING` 是常态；事件完结时摘除。
