...
- **人物称呼一律照抄研究文件，写完必须自查（用户裁定，2026-07-21）：** 草稿里每一个人物称呼都必须能在研究文件里**逐字**找到。交稿前把草稿中出现的人名/代称逐个回研究文件里搜一遍，搜不到的就是你自己造的，改回研究文件的写法。写手没有发明称呼的权限——下面两条只是这条规则的展开。
...
- **只收事件，不收评论（用户裁定，2026-07-21）：** 任何人对事件的评论都不进草稿——匿名网民留言、评论区回复、转发评论、微博热评、境外媒体转录的网民言论，与具名专家评论同等对待。这条**不限于 `## 舆论` 节**，`## 概述`/`#### 时间线`/`## 相关内容` 同样适用：评论有出处、能逐字引、可以套灰字引用，并不使它可写——"有来源支撑"只是必要条件。研究文件收录了评论也不例外，写手自行剔除。唯一例外：**该言论本身就是事件的加害行为或被追责对象**（造谣内容、网暴言论、骚扰言论、被警方处罚或被平台处置的言论），此时它是事实不是评论，照写。
- **转发帖不进文章、不进来源（用户裁定，2026-07-21）：** 微博等社交平台的**转发帖**不能写进正文、也不能出现在 `## 信息来源`——例外是转发者本人为**当事方（含家属）、媒体机构或官方机构**。研究文件里若某说法只挂着普通网民/自媒体博主的转发帖（来源行形如"A（转发 B 内容）"），该说法整句删除、来源行删除，不留"未见证实"式的缺口说明。**原帖可以用**：当事方自述、博主首发爆料等原创帖照写照引。
- **转引自媒体的说法＝没有来源，不许用"据报道"洗白（用户裁定，2026-07-21）：** 正规媒体的报道里凡写明"据自媒体X报道""某公众号称""网传"的内容，真实出处仍是自媒体，媒体只是转引，不构成媒体自采、不构成独立信源——这类说法整句删除，不进草稿。**尤其禁止写成含糊的"据报道""据媒体报道"**：那是把自媒体转述洗成有采编把关的媒体报道，读者无从分辨。推而广之，凡是你写不出具体归属的说法，就是不能写——"据报道"不是归属。反例：某案"因未满足法官的要求，王女士最终输掉了离婚案件"，实际链条是自媒体"四海瞭望"→新唐人电视台转引→研究文件标成"（来源：新唐人电视台转引报道）"→草稿写成"据报道"，用户直接删句。
...
- **不许删 review 文件里的 `[USER]` 注释（2026-07-21）：** "草稿里不得残留 [USER]/[REVIEWER] 注释"只管**草稿**。review 文件是工作留痕，用户的裁定原文必须原样留在 `## 人类的裁定` 里——你只能在其后追加"（已应用，见问题K处理行）"，**不得删除、改写或替换成指针**。裁定被抹掉后，下一轮评审和下一个写手就看不到用户当初为什么这么定，同样的问题会被重新提一遍。
...
- `PING` — **事件**还没走完，插眼等后续进展。文章事实已站得住，只是结局未定（案件待判、程序在途、平台处置未出）。已发表文章带 `PING` 是常态。运维方式：定期把挂 `PING` 且已满一个月的文章跑一遍，看有无后续；有后续就写新文章，与原文互挂 `## 前情`/`## 后续` 链接；后续出现且事件已完结时摘除 `PING`。
