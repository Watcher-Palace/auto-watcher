...
返回的**新文件**；派单里 `draft_path` 指向的当前版草稿全程只读，任何情况下都不在它上面
编辑——包括"先改着看看再另存为新版"。`_pipeline/draft/` 里的在途草稿常常尚未提交 git，
原地改一旦没补救回来，上一版就永久丢失、没有基线可比对，评审文件里那些 `原文：` 逐字锚点
也随之失效。（例：260605-2 v1 被原地改后靠写手自觉还原；链条见 casebook）
...
- **标题自己写，不要照搬内部标签（用户裁定，2026-07-22，多次复现）：** 派单里的 `title`、研究文件名、事件账本标题都是**内部索引标签**，只供我们内部对号，**读者永远看不到它**。frontmatter 的 `title` 你要**另写一个信息完整、能独立读懂的标题**：点明关键当事方、发生了什么、以及最核心的进展或落点，让没看过内部标签的人一眼知道这是什么事——内部标签往往只是四五个字的类目速记，直接搬去当标题等于什么都没说。反向也要收住：**这三样之外的东西不进标题**——法院说理与认定依据、关系过程修饰等一律留给正文，把整条认定链塞进去会长到读不动（例：260716-7 原样照搬内部标签、260108-2 标题含"法院认定其应知对方系幼女"被删，两个方向的失败；改写链见 casebook）。标题仍须服从下面这条：只陈述事实，不缀舆论反应词。（linter 会拦/警告）
- **标题以加害人为主语，不写受害人被动句（用户裁定，2026-07-31）：** 标题写"**谁对谁做了什么**"，不写"受害人承受了什么"——"女子遭前男友伪装快递上门杀害"要写成"男子伪装快递上门杀害前女友"。被动句**即使点名了施动者**也不行：受害人被摆在主语位，加害行为就被写成了她的遭遇，加害人退成修饰语。**例外：加害人未知、未归案或无法特指时**（"女生宿舍遭陌生男子闯入"）允许被动，但**施动主体必须留在标题里**——"女子被杀害""女生遭猥亵"这类把加害人整个抹掉的写法任何情况下都不许用。
- **标题不追加舆论反应（用户裁定，2026-07-21）：** frontmatter 的 `title` 只陈述发生了什么。已经把事实说完之后再缀"引争议""引质疑""引发关注""惹众怒"这类概括舆论反应的措辞，一律删掉——那是评论不是事实，和正文里"舆论对此提出质疑"属同类措辞（反例：法官猥亵案的"行政立案引争议"，事实是猥亵与行政立案，"引争议"是加上去的）。**例外：争议本身就是事件主体时照写**——署名归属之争、广告风波、游戏立绘争议这类事件，去掉"争议/风波"就描述不出发生了什么，此时它是事件名称的一部分，不是追加的反应（已发布例：名创优品偷窥女性广告风波）。判断法：删掉该词后标题是否还说得清事件——说得清就删，说不清就留。（linter 会拦/警告）
...
- **带色引文必须逐字回查，基准是 `## 信息来源`（用户裁定 2026-07-28；比对基准 2026-07-31 更正）：** `source/_drafts/template.md` 规定"灰色 = 当事人/法院/官方的逐字引用"，其反向义务同样成立：**凡要上灰色的文字，必须逐字命中研究文件 `## 信息来源` 节的引文摘录（或该来源原文）**，交稿前逐段回查。**基准只认 `## 信息来源`，不认 `## 事实`／`## 当事方`**——后两节是研究阶段的叙述性转写，可能已经把官方原话改过一遍，照它抄照样"对得上"，抄到的却是转写版。`## 信息来源` 里没有该句的逐字摘录，就按缺口上报等研究补齐，不许自行上灰色。红字若是对官方通报、法院认定、平台处置结论的转述，二选一：改成灰字逐字引用，或去掉颜色写成明确的转述——**不得以"近乎逐字、又有几处改写"的形态呈现**，那会让读者把你的改写当成官方原话。（例：260605-4 灰字把法官的第三人称转述包装成直接引语，原始报道均无引号；链条见 casebook）
...
- **半匿名代称已经是化名，原样沿用（用户裁定，2026-07-20，2026-07-21 重申）：** 报道给出的"白女士""高某某""小林"这类只有姓氏/化姓/昵称的称呼，本身就是报道做过的匿名处理，**直接照抄**，标注"（报道使用化名）"。不要因为"白""高"是真姓就套用下一条把它换成全名式化名——那是二次化名，切断读者与原报道的对应关系，是评审必退的错误（例：260716-7"白女士"被二次化名为"林悦"；链条见 casebook）。此规则适用于事件中的所有当事方，不限于受害人。
- **受害人必须用化名（用户裁定，2026-07）：** 仅当来源给出的是**完整真实姓名**（姓+名，如"张桂芳"）时才替换——包括受害人或其家属自行公开真名的情形——替换后在首次出现处标注"（化名）"。草稿任何位置（含引文、账号名、话题名）都不得保留受害人真名，引文中出现时同样替换。同一事件内化名前后必须一致。来源已经是化名或半匿名代称的，走上一条，不要再换。
  - **来源行标题里的真名用全角星号打码，不用化名（用户裁定，2026-08-04）：** `## 信息来源` 的斜体位必须是文章**真实标题**、逐字照抄，与正文的化名规则直接冲突（一篇讲"媒体实名披露即侵害隐私"的文章，自己的来源行却把她的名字再印一遍）。裁定的解法是**打码**而非替换成化名：把标题里的受害人**全名**逐字替换为等长的**全角** `＊`（`李欣莳` → `＊＊＊`），标题其余部分一字不改。**必须用全角 `＊`，不许用半角 `*`**——半角星号是来源行斜体的定界符，写进标题会把标题截断，linter 认不出。正文用化名、来源行标题打码，两者并存不算不一致。研究文件保留真名（研究阶段不脱敏），`linter.py --research` 的来源行核对已对打码位做通配处理，不会因此 FAIL。（例：260717-2 澎湃新闻《高校公告：＊＊＊同学，不当行为造成恶劣负面影响，拟开除学籍》）
- **臆想被推翻就整句删除，不留"未见 xxx"（用户裁定，2026-07-20）：** 初稿写进去的内容如被评审/研究更正为无来源支撑或查证失败，正确处理是把该内容整句删掉，而不是改写成"公开报道未确认……""未见证实……""其本人未就此回应"之类的存在性说明——那等于把模型自己臆想出来的话题留在文章里，让读者以为这是一个真实存在的信息缺口。例外：该缺口本身构成重要事实（如刑事程序是否启动、官方是否发布通报、关键处置结果是否公开）时，才保留缺口说明。研究文件本来就有的缺口陈述不受此限。**初稿要主动写"某方未回应／未就此反驳"之前，先回研究文件查一遍**：该方的回应常与你引用的那句话出自**同一篇报道**，只是没被摘在事实节的相邻位置，漏查就会写出一句与自己正文别处直接矛盾的存在性判断。
...
- **当事人的心境自述不进正文（用户裁定，2026-07-31）：** 当事人讲述自身**感受、心境、动机**的话（"只想逃离这个家，越远越好""哪怕坐牢，我也不后悔"），说的不是"发生了什么"，是抒情材料不是事件事实——**逐字可引、来源可靠都不构成写进去的理由**，与"只收事件，不收评论"同样处理，整句剔除。判据看该句陈述的是**可核实的行为与经过**（谁在何时何地对谁做了什么），还是**内心状态**（感受、悔与不悔、想不想）：前者照写，后者剔除。当事人复述事实经过的话不受此限（如"他威胁我说要杀我全家"是行为陈述，照写照引）。
...
- **归属写不具体就不能写，但别把"非官方"一律当成没来源（用户裁定 2026-07-21，2026-07-31 分层细化）：** 判据是**能不能写出具体归属**——谁、何时、在哪说的。自媒体**首发的事实主张**（"据自媒体X报道""某公众号称""网传"）出处就是它自己，不构成独立信源，整句删除、不进草稿；尤其禁止写成含糊的"据报道""据媒体报道"给它套壳。但自媒体**转录的可核验原始材料**（当事方原帖、判决书、官方通报等）不在此列：研究文件按新规已尽量取原件入库、取不到的会写明完整链条，这类内容**可以写**，照研究文件登记的归属如实写出（原作者与原发日期；确系二手转录的注明转录方），不要因为链条里出现自媒体就整句删掉。（完整链条见 casebook：260716-7）
...
- `公职人员`：不含教师——教师不属于公职人员。
- **地区标签只标境外（用户裁定，2026-07-21）：** `location` 组只用于国别与跨国属性（`德国`、`跨国`…）。**国内案件一律不加地区标签**，省市名不进注册表——"有地区标签"因此等于"境外案件"。国内案发地写在正文里即可，不要为它提标签提案。**港澳台属国内**：不加地区标签，也不算 `跨国`。**`跨国` 限事件本身跨越国境**——多国有组织网络、跨国安排、跨境流转才挂；单一加害人、单一受害人、发生在某国境内的个案只挂国别标签，**事发地在境外不等于案件具跨国属性**（两次裁定：260708-6 在韩个案只挂 `韩国`、260605-3 境外街头互动但争议全在国内平台展开，均不挂 `跨国`）。
