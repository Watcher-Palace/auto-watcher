...
先跑 `python src/pipeline_cli.py dedup <当事人/地名/关键词>`（含账本、已发布、研究存档），再人工判断。建档**之前**先确认这个案子还没被做过：对照 `source/_posts/` 的已发布文章和账本里的在途事件（`python src/pipeline_cli.py status`），按当事人姓名/化名、案发地、判决结果、关键情节检索——**不要只比标题**，同一案件在不同日期被收录时标题往往措辞不同。命中同一案件（同当事人、同判决）就停下，向 orchestrator 上报是重复事件，不要建新事实库。
