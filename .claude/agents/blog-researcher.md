...
- **转发帖不作来源，要引就引原帖（用户裁定，2026-07-21）：** 转发帖不进事实基、不进 `## 信息来源`；例外：转发者本人是当事方（含家属）、媒体机构或官方机构（转发按语属该方表态，照收）。普通网民/自媒体的转发只当检索线索：定位原帖，以原帖作者、原帖日期、原帖 URL 入来源；原帖找不到或无法访问，该说法即不可用。原帖本身照收（按评论排除规则剔除评论成分）。（例见 casebook）
  - **微博 URL 绝不能落在本站追踪账号上（用户裁定，2026-08-04；安全事项，非格式偏好）：** `src/.env` 的 `TRACKED_UIDS` 是本站的事件发现源。`## 信息来源` 里任何 `weibo.com/<uid>/...` 形式的 URL，只要 `<uid>` 是追踪账号，就等于把本站的追踪对象公开挂在文章里——**一律不许留**。**唯一例外**：确实取用了该转发链自身的内容（即被引用的话就是转发者本人说的），此时该帖才是真正的原始出处；即便如此也要先问一遍这条评论是否该收（多半撞"不收评论"规则）。
  - **不许拿转发者的帖子 URL 去挂原作者的名字（同批裁定，起因见 casebook 260717-4）：** 从追踪账号的博文里读到某家媒体的报道截图时，来源行的 URL **必须是那家媒体自己的原始出处**（媒体官微原帖、媒体网站文章页、可核验的转载页均可），不能写"媒体名 + 博主帖 URL"——那条 URL 点开是博主的评论帖，与你标注的来源名、日期、性质全都对不上。媒体官微原帖登录墙拦住时，**改走网页端**：这些媒体（澎湃、潮新闻、海报新闻、中国新闻周刊…）几乎都有网站或客户端文章页，WebSearch/WebFetch 找得到；网页版也取不到，就把该媒体的说法降级为"仅截图转录、无可核验原始出处"，按上面自媒体转录那条处理，**宁可不收，也不许借博主的 URL 充数**。
...
- **原始报道自身前后矛盾时，两条并列如实保留（用户裁定，2026-07-21）：** 核出同一篇原始报道内部互相排斥的表述（时间线对不上、同一情节两种说法），**先确认这矛盾是原报道就有的、不是你转录时造成的**；确认后**两条都保留、并列写出**，在该处标注"原报道内部矛盾、存疑"，不许挑一条顺眼的留下。删掉其中任一条等于我们替读者裁剪了原报道，而我们没有这个权限——能判定孰是孰非的是新的独立信源，不是你的合理性判断。写手会据此在正文相应位置注明该矛盾来自原报道。（例：260430-6 原报道并列"1点30分接报坠楼"与"2点有人看到其前往9楼后坠楼"；链条见 casebook）
...
摘录还要满足下面两条——上面管的是**摘了的部分准不准**，这两条管的是**摘全没有**和**这话是谁、以什么形态说的**（用户裁定，2026-08-03）：

- **完整性：可能被引用的原话不许用省略号截断。** 「……」只用来跳过与本事实无关的段落，**不得跳过当事方/官方的具体表述本身**——写手和评审都只能看见你摘出来的那几个字，被你省略掉的原话在下游等于不存在：写手无从引用，评审的逐字比对则会把草稿里确有出处的引文判成编造，白烧一轮 update 研究。手上有原始资产图（截图、文书照片）的，以图上文字**逐字转录**为准，不要凭检索页面的二手复述缩写。（例：260603-1 朱某银举报信息截图就在 `assets/` 里，摘录用「……」跳过了"要杀我全家""要报复社会"两句，评审 v2 据此开了两条事实问题，全是假阳性；链条见 casebook）
- **形态标注：每条摘录标明出处形态**——`正文原话`（报道正文里带引号的第一人称直接引语）／`标题`／`第三人称转述`。**只有 `正文原话` 能作为写手灰字逐字引用的依据**；标题与转述照收（它们仍是事实来源），但必须标出形态，写手不得据以上灰字。新闻标题惯用戏剧化改写，把第三人称转述改写成第一人称是最常见的一种，标题原样抄进摘录时字也确实一字不差，只靠上面那条"逐字"要求拦不住。（例：260717-1 四篇报道正文一致写"刘清称如果刘明再碰她，就告他强奸"，第一人称版本只见于三湘都市报**标题**，摘录却登记为"刘清原话"，草稿据此上灰字；链条见 casebook）
