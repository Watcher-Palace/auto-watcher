...
- **不收评论（用户裁定，2026-07-21）：** 事实节不收任何人对事件的评论——匿名网民留言、评论区回复、转发评论、微博热评、境外媒体转录的网民言论，与具名专家评论同等对待。写手禁止把评论写进正文，事实基里留着只会诱使它违规。唯一例外：**该言论本身就是事件的加害行为或被追责对象**（造谣内容、网暴言论、骚扰言论、被警方处罚或被平台处置的言论），此时它是事件事实，照收并注明发布者与出处。舆论规模只收可核实的具体数字（阅读量/讨论量/转发量/评论量/投票结果），不做"舆论普遍认为"这类定性。
- **转发帖不作来源，要引就引原帖（用户裁定，2026-07-21）：** 微博等社交平台的**转发帖**不能进事实基、也不能进 `## 信息来源`——例外是转发者本人为**当事方（含家属）、媒体机构或官方机构**，此时转发按语是该方的表态，属事实，照收。普通网民/自媒体博主的转发只能当**检索线索**：顺着它定位被转发的**原帖**，以原帖作者、原帖发布日期、原帖 URL 入来源；原帖找不到或无法访问，该说法即不可用，当它不存在。**原帖本身可以收**（当事方自述、博主首发爆料等），照常注明发布者与出处，并按上一条的评论排除规则剔除其中的评论成分。历史反例：某案把博主 A 转发博主 B 的内容当来源，来源行只能写"A（转发 B 内容）"，读者无从对应原始出处。
- **转引自媒体的说法不算独立信源（用户裁定，2026-07-21）：** 媒体报道里写明"据自媒体X报道""某公众号称""网传"的内容，真实出处是自媒体，媒体只是转引——它不满足"核心事实需两个独立信源"，也**不能在事实条目上标成"（来源：某某电视台）"**了事：那会让写手误以为是媒体自采，进而写成含糊的"据报道"。**这类说法不进事实节。**确需留作线索时，放在条目末尾用括号写明完整链条（自媒体名→转引媒体）并注明"仅自媒体转述，无媒体自采或官方确认，写手不得使用"。反例：某案"因未满足法官要求，王女士最终输掉离婚案件"，链条为自媒体"四海瞭望"→新唐人电视台转引，被标为"（来源：新唐人电视台转引报道）"，写手据此写进正文，用户删句。
...
- **怎么抓**：普通网页图用 `curl -L --referer <页面URL> -o <目标路径> <图片URL>`（多数站点防盗链，缺 Referer 会拿到占位图，下载后检查文件大小与类型）；微博单帖的图先跑 `src/venv/bin/python src/wbfetch.py <帖子URL>` 取其返回的 `image_urls`，再逐个下载。
