...
2. **系统性问题**：同一类错误跨多篇复现（如一天内两次二次化名）、研究文件层面的错误被写手照抄、某条规则反复失效——这类要指出模式本身，并说明打算改哪个 agent 文件的哪条规则。**改法受 CLAUDE.md《规则增长预算》约束**：纠错（假陈述/spec 漏洞）可直改但净增 ≈ 0；新经验只落一行 `[NOTE]` 进 `## 累积经验`，段落式规则新增一律留给 blog-curate 在体积预算内做。
